...
public class PlayerSounds : MonoBehaviour
{
    ...
    void kart_sounds()
    {
        ...
        if (playerscript.currentspeed >= 5 && !playerscript.GLIDER_FLY && !item_script.isBullet)
        {
            ...
            {
                kartSound.time = Mathf.Lerp(kartSound.time, 0.5f, 3 * Time.deltaTime);
                kartIdle.Stop();

            }
            if (playerscript.currentspeed > 10 && playerscript.currentspeed <= 20)
            {
                kartSound.time = Mathf.Lerp(kartSound.time, 1f, 4 * Time.deltaTime);
                kartIdle.Stop();
            ...
            {
                kartSound.time = Mathf.Lerp(kartSound.time, 3, 4 * Time.deltaTime);
                kartIdle.Stop();
    ...
    public bool Check_if_playing()  //this method returns a bool, and checks if any mario sounds are already playing, so if-statements can decide whether to or not to play a new sound
    {
        ...
        {
            if (Mario_Boost_Sounds[i].isPlaying || effectSounds[18].isPlaying)
                return false;
        }
        return true;
